...
def get_supabase() -> Client:
    # Serwer sam wyciąga hasła z Twojego sejfu!
...
if st.session_state.zalogowany_uzytkownik is None:
    ...
    with tab_logowanie:
        ...
        if st.button("🔓 Zaloguj", use_container_width=True):
            try:
                # Pytamy prawdziwą bazę danych o użytkownika
    ...
    with tab_rejestracja:
        ...
        if st.button("📝 Utwórz konto Kursanta", use_container_width=True):
            if len(rej_haslo) < 4:
                ...
                try:
                    # Sprawdzamy, czy mail już istnieje w Supabase
...
# ==========================================
# Pobieramy zawsze NAJŚWIEŻSZE dane z bazy (np. po zużyciu tokena)
...
if user_data['rola'] == "Instruktor":
    ...
    # Pobieramy wszystkich kursantów z Supabase
    ...
    if not kursanci:
        ...
        # Lista wyboru dla instruktora
        opcje_kursantow = {k['email']: f"{k['imie']} ({k['email']})" for k in kursanci}
        wybrany_email = st.selectbox(
            "Wybierz kursanta:",
            options=list(opcje_kursantow.keys()),
            format_func=lambda x: opcje_kursantow[x]
        )
            ...
            if not api_key:
                ...
                with st.spinner('AI analizuje i wysyła do bazy...'):
                    try:
                        ...
                        nowe_zadanie = f"Wygenerowano awaryjnie: Zwróć uwagę na płynność gazu w zakrętach. (Błąd: {e})"

                    # Aktualizujemy bazę Supabase o nowe zadanie dla ucznia!
    ...
    if sim_file:
        if st.button("Wyślij do chmury (Zużywa 1 token)"):
            if user_data['tokeny'] > 0:
                # Odejmujemy token i wysyłamy info do Supabase
                ...
                time.sleep(2) # Sztuczne opóźnienie dla ładnego efektu
                st.rerun() # Odświeża stronę i zaciąga nowy stan tokenów z bazy
            else:
