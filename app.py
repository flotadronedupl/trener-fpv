    ...
    if premium:
        ...
        with t2:
            if has_acc:
                ...
                st.info("Brak danych z akcelerometru w tym logu, aby wygenerować wykres G-Force.")
    ...
    with col2:
        ...
        with t2:
            ...
            if st.button("Zarejestruj się"):
                ...
                st.success("Konto zostało pomyślnie założone! Możesz się teraz zalogować.")
        st.markdown("</div>", unsafe_allow_html=True)
    st.stop()
...
if user_data['rola'] == "Instruktor":
    ...
    with col_nav:
        ...
        with col_t2:
            ...
            if st.button("ZASIL"):
                ...
                st.toast("Konto zostało pomyślnie zasilono.", icon="🟢")
                time.sleep(1)
                st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col_main:
        ...
        with c_upl: log_file = st.file_uploader("Wgraj plik czarnej skrzynki (BBL/CSV)", type=['bbl', 'csv'], label_visibility="collapsed")
        with c_vid: vid_link = st.text_input("Opcjonalny link do nagrania (YouTube/DVR)", placeholder="https://...")

        df_active = None
        if log_file:
            with st.status("Trwa ekstrakcja danych telemetrycznych...", expanded=False) as status:
                ...
                status.update(label="Dane zostały poprawnie zdekodowane.", state="complete", expanded=False)
        ...
        if df_active is not None:
            ...
            if st.button("GENERUJ RAPORT AI"):
                if init_ai():
                    ...
                    Poziom zaawansowania: {inst_skill}. Styl lotu: {inst_ind}.
                    
                    DANE LOTU:
                    - Płynność Roll: {stats['jr']:.2f} (niskie wartości oznaczają płynne wejścia w zakręty)
                    - Płynność Pitch: {stats['jp']:.2f}
                    - Przeciążenia G-Force (Maksymalne): {stats['max_g']:.1f} G
                    
                    ZADANIE:
                    1. "ocena": Skala 1-10.
                    2. "diagnoza": Skomentuj styl lotu i przeciążenia w sposób profesjonalny. Jeżeli G-Force jest wysokie (>3G), zaznacz, że dron wykonywał agresywne manewry lub ostre nawroty. Dopasuj słownictwo lotnicze do poziomu: {inst_skill}.
                    3. "zadanie": Zaproponuj jedno konkretne zadanie treningowe do wykonania podczas następnego lotu.
                    
                    ZWRÓĆ TYLKO I WYŁĄCZNIE CZYSTY JSON: {{"ocena": 8, "diagnoza": "Cześć...", "zadanie": "Spróbuj..."}}
                    """
                    raw = generate_intel(prompt)
                    try:
                        js = json.loads(raw.replace("```json","").replace("```","").strip())
                        st.session_state.instructor_draft = f"### Analiza lotu: {inst_ind}\n**OCENA WYDAJNOŚCI:** {js['ocena']}/10\n\n**KOMENTARZ TRENERA:**\n{js['diagnoza']}\n\n**CEL TRENINGOWY NA NASTĘPNY LOT:**\n{js['zadanie']}"
                        st.session_state.temp_metrics = stats
                    except: st.error("AI napotkało problem przy analizie. Spróbuj ponownie.")
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        if st.session_state.instructor_draft:
            ...
            final_rep = st.text_area("Edytor tekstu", value=st.session_state.instructor_draft, height=250, label_visibility="collapsed")
            ...
            if st.button("ZATWIERDŹ I WYŚLIJ DO PILOTA"):
                ...
                st.success("Raport został pomyślnie wysłany do systemu.")
                time.sleep(1)
                st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
...
# ==========================================
    ...
    if st.session_state.flow_state == 'launchpad':
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("<div class='bento-card'><h3>🚁 LOT RZECZYWISTY</h3><p class='mono-text'>Analiza logów z drona (.BBL)</p></div>", unsafe_allow_html=True)
        ...
        if st.session_state.industry_select:
            ...
            skill = st.select_slider("Wskaż swój poziom zaawansowania, by sztuczna inteligencja mogła dostosować ocenę:", options=["Początkujący", "Średniozaawansowany", "Ekspert"], value=st.session_state.skill_select, label_visibility="collapsed")
                    ...
                    if user_data.get('tokeny', 0) >= cost:
                        with st.status("Trwa analizowanie fizyki lotu...", expanded=True) as status:
                            ...
                            if csvs:
                                ...
                                if init_ai():
                                    prompt = f"""
                                    Jesteś zaawansowanym systemem instruktorskim FPV. Pilot to {user_data['imie']}.
                                    Poziom: {st.session_state.skill_select}. Cel lotu: {st.session_state.industry_select}.
                                    
                                    DANE Z CZARNEJ SKRZYNKI:
                                    - Wskaźnik szarpania (Roll/Pitch): ~{stats['jr']:.2f} (niskie wartości oznaczają płynne ruchy drążków).
                                    - Maksymalne przeciążenie (G-Force): {stats['max_g']:.1f} G.
                                    
                                    ZADANIE:
                                    1. "ocena": 1-10.
                                    2. "diagnoza": Wykorzystaj profesjonalny, techniczny styl odpowiedni dla poziomu: {st.session_state.skill_select}. Zinterpretuj wskaźniki G-Force i płynności drążków.
                                    3. "zadanie": Podaj jeden konkretny plan treningowy na następne podłączenie zasilania do drona.
                                    
                                    ZWRÓĆ WYŁĄCZNIE JSON: {{"ocena": 8, "diagnoza": "Cześć...", "zadanie": "Skup się na..."}}
                                    """
                                    ...
                                    try:
                                        ...
                                        time.sleep(1)
                                        st.rerun()
                                    except: st.error("Wystąpił problem z systemem AI. Spróbuj ponownie.")
